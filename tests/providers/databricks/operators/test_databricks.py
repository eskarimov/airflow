...
from airflow.providers.databricks.operators.databricks import (
    DatabricksRunNowAsyncOperator,
    DatabricksRunNowOperator,
    DatabricksSubmitRunAsyncOperator,
...
class TestDatabricksSubmitRunAsyncOperator(unittest.TestCase):
    @mock.patch('airflow.providers.databricks.operators.databricks.DatabricksHook')
    def test_execute_task_deferred(self, db_mock_class):
        ...
        op = DatabricksSubmitRunAsyncOperator(task_id=TASK_ID, json=run)
        db_mock = db_mock_class.return_value
        db_mock.submit_run.return_value = 1
        db_mock.get_run_state.return_value = RunState('TERMINATED', 'SUCCESS', '')
    ...
    def test_execute_complete_incorrect_event_validation_failure(self):
        event = {}
        op = DatabricksSubmitRunAsyncOperator(task_id=TASK_ID)
        with pytest.raises(AirflowException):
            op.execute_complete(context=None, event=event)
...
class TestDatabricksRunNowAsyncOperator(unittest.TestCase):
    @mock.patch('airflow.providers.databricks.operators.databricks.DatabricksHook')
    def test_execute_task_deferred(self, db_mock_class):
        """
        Test the execute function in case where the run is successful.
        """
        run = {'notebook_params': NOTEBOOK_PARAMS, 'notebook_task': NOTEBOOK_TASK, 'jar_params': JAR_PARAMS}
        op = DatabricksRunNowAsyncOperator(task_id=TASK_ID, job_id=JOB_ID, json=run)
        db_mock = db_mock_class.return_value
        db_mock.run_now.return_value = 1
        db_mock.get_run_state.return_value = RunState('TERMINATED', 'SUCCESS', '')
    ...
    def test_execute_complete_incorrect_event_validation_failure(self):
        event = {}
        op = DatabricksRunNowAsyncOperator(task_id=TASK_ID, job_id=JOB_ID)
        with pytest.raises(AirflowException):
            op.execute_complete(context=None, event=event)
